...
def send_feedback_notification(feedback_id):
    ...
    try:
        ...
        params = {"chat_id": chat_id, "text": feedback_text}
...
@shared_task
def send_vacancy_application_notification(application_id):
    """
    Отправляет уведомление о новом отклике на вакансию в Telegram
    """
    try:
        from app_home.models import VacancyApplication
        ...
        application_text = (
            ...
            f"Вакансия: {application.vacancy.title}\n"
            f"Имя: {application.name}\n"
            f"Возраст: {application.age}\n"
            f"Телефон: {application.phone}\n"
            f"Стаж работы: {application.experience_years} лет\n"
            f"Опыт работы: {application.work_experience}\n"
            f'Дата отклика: {application.created_at.strftime("%d.%m.%Y %H:%M:%S")}\n'
        )

        # Отправляем сообщение в Telegram
        url = f"https://api.telegram.org/bot{bot_token}/sendMessage"
        params = {"chat_id": chat_id, "text": application_text}
        ...
        return f"Уведомление об отклике на вакансию #{application_id} отправлено"

    except VacancyApplication.DoesNotExist:
        return f"Объект VacancyApplication с id {application_id} не найден"
    except Exception as e:
        return f"Ошибка при отправке уведомления: {str(e)}"
